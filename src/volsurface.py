...
class VolSurface(BaseEstimator, ABC):
    ...
    def plot(self, ax=None, resolution=10, **kwargs):
        ...
        surf = ax.plot_surface(d, m, v, **kwargs)
        ax.set_xlabel("Delta")
...
class GridInterpVolSurface(VolSurface):
    ...
    def _fit(self, delta, maturity, vol):
        ...
        for i in range(len(vol)):
            ...
            if 0 <= d_idx < len(self.delta_grid) and 0 <= m_idx < len(self.maturity_grid):
        ...
        self._interp = RectBivariateSpline(
            self.delta_grid, self.maturity_grid, grid_vol,
...
class KernelVolSurface(VolSurface):
    ...
    def _fit(self, delta, maturity, vol):
        ...
        if self.bandwidth is None:
            n = len(delta)
            self.bandwidth = np.array([np.std(delta), np.std(maturity)]) * (n ** (-1 / 5)) * 1.06

        self.model = KernelReg(
            endog=vol, exog=np.column_stack((delta, maturity)),
            var_type='cc', bw=self.bandwidth
        )
